...
export async function runPricesIntersect(
  ...
  const btcPrices = prices.filter((p: any) => p.pair === 'BTCUSDT')
  ...
  const spreads = btcPrices.map((p: any) => p.net_spread_pct)
  const allSame = spreads.every((v: number) => Math.abs(v - spreads[0]) < TOLERANCE)
  ...
  const hasIntersectEntry = btcPrices.some(
    (p: any) => Math.abs(p.net_spread_pct - (-ALL_IN_COST)) < TOLERANCE
  )
